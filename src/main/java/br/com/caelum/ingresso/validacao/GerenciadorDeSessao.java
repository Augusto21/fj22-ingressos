...
public class GerenciadorDeSessao {
	...
	private boolean horarioIsConflitante(Sessao sessaoExistente, Sessao sessaoNova) {
		...
		boolean sessaoNovaTerminaAntesDaExitente = terminoSessaoNova.isBefore(inicioSessaoExistente);
		boolean sessaoNovaComecaDepoisDaExitente = terminoSessaoExistente.isBefore(inicioSessaoNova);

		if (sessaoNovaTerminaAntesDaExitente || sessaoNovaComecaDepoisDaExitente) {
			return false;
		}
		return true;
	}
	...
	private LocalDateTime getTerminoSessaoComDiaDeHoje(Sessao sessao) {
		LocalDateTime inicioSessao = getInicioSessaoComDiaDeHoje(sessao);
		return inicioSessao.plus(sessao.getFilme().getDuracao());
	}
